 ...
 *
 * $Id: TestMemberCCSD.cs 1232 2008-03-14 05:36:00Z mm $
 */
...
namespace Gentle.Framework.Tests
{
	...
	/// MemberCCSD class. The database must have been created and populated with the 
	/// supplied test data for the tests to work.
	/// The MemberCCSD class is like the Member class with added concurrency control
	/// and soft deletion.
	/// </summary>
	[TestFixture]
	public class TestMemberCCSD
	{
		private MemberCCSD m1, m2;
		...
		public void TestCRUD()
		{
			if( GentleSettings.ConcurrencyControl )
			{
				...
				CacheManager.Clear();
				m1 = new MemberCCSD( list.Id, "John Doe", "john@doe.com" );
				// insert
				m1.Persist();
		...
		public void TestCount()
		{
			if( GentleSettings.ConcurrencyControl )
			{
				...
				sr = Broker.Execute( sb.GetStatement( StatementType.Count, typeof(MemberCCSD) ) );
