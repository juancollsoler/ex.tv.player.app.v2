 ...
 *
 * $Id: TestMemberSD.cs 1232 2008-03-14 05:36:00Z mm $
 */
using System;
using Gentle.Common;
...
namespace Gentle.Framework.Tests
{
	...
	/// MemberSD class. The database must have been created and populated with the 
	/// supplied test data for the tests to work.
	/// The MemberSD class is like the Member class with added soft deletion.
	/// </summary>
	[TestFixture]
	public class TestMemberSD
	{
		private MemberSD m1, m2;
		...
		{
			m1 = new MemberSD( list.Id, "John Doe", "john@doe.com" );
			// insert
			m1.Persist();
		...
		public void TestCount()
		{
			...
			sr = Broker.Execute( sb.GetStatement( StatementType.Count, typeof(MemberSD) ) );
