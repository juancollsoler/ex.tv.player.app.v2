 ...
 *
 * $Id: SybaseAnalyzer.cs 1232 2008-03-14 05:36:00Z mm $
 */
...
{
	public class SybaseAnalyzer : GentleAnalyzer
	{
		public SybaseAnalyzer( IGentleProvider provider ) : base( provider )
		{
		}
		...
		private const string selectTables =
			"select systable.*, sysuserperm.user_name from systable inner join sysuserperm on creator=user_id " +
			"where user_name <> 'SYS' and user_name <> 'dbo' and user_name not like 'rs%'";
		// get all columns
		private const string selectColumns =
			"select sys.syscolumn.*, sys.sysdomain.* from sys.systable join sys.syscolumn on " +
			"	sys.systable.table_id=sys.syscolumn.table_id " +
			"join sys.sysdomain on sys.syscolumn.domain_id=sys.sysdomain.domain_id " +
			"where sys.systable.table_id={0} order by sys.syscolumn.column_id"; // table id
		// get all foreign key constraints
		private const string selectForeignKeyConstraints =
			"select c.constraint_name 'constraint', fc.column_id, ft.table_name 'PT', pt.table_name 'FT', " +
			"pc.column_name 'PK', fc.column_name 'FK' from sys.sysforeignkey as fk " +
		...
		private const string selectPrimaryKeyConstraints =
			"select current database user_name, t.table_id,table_name,column_name,column_id " +
			"from SYS.SYSTABLE as t,SYS.SYSCOLUMN as c,SYS.SYSUSERPERM as u where " +
		...
		public override void Analyze( string tableName )
		{
			try
			{
				...
				SqlResult sr = broker.GetStatement( selectTables ).Execute();
