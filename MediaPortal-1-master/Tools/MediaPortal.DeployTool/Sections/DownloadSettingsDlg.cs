...
namespace MediaPortal.DeployTool.Sections
{
  public partial class DownloadSettingsDlg : DeployDialog
  {
    ...
    {
#if DEBUG
      MessageBox.Show(String.Format("arch = {0}, langA = {1}, langB = {2}", arch64 ? "64" : "32",
                                    listViewLang.FocusedItem.SubItems[1].Text, listViewLang.FocusedItem.SubItems[2].Text));
#endif
      InstallationProperties.Instance.Set("DownloadArch", arch64 ? "64" : "32");
      InstallationProperties.Instance.Set("DownloadThreeLetterWindowsLanguageName",
                                          listViewLang.FocusedItem.SubItems[2].Text);
