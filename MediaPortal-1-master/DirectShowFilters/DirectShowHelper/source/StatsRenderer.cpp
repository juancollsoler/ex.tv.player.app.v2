...
void StatsRenderer::DrawStats()
{
  ...
  RECT rc = {m_pPresenter->m_iVideoWidth/32, m_pPresenter->m_iVideoHeight/8, m_pPresenter->m_iVideoWidth, m_pPresenter->m_iVideoHeight};
    ...
    strText.Format(_T("Rptd FPS: %.3f, Detd FPS: %.3f | SDur: %+5.2f ms, ST_SD: %+5.2f ms, LJ: %d | ver: %d"),  
      ...
      (m_pPresenter->m_SampDuration/10000.0), (m_pPresenter->m_DetectedFrameTimeStdDev/10000.0),
      m_pPresenter->m_LowSampTimeJitterCnt, DSHOWHELPER_VERSION);
    DrawText(rc, strText);
    OffsetRect(&rc, 0, TextHeight);
