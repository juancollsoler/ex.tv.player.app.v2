...
namespace MediaPortal.Player.Teletext
{
  internal class TeletextReceiver
  {
    ...
    private TeletextEventCallback eventCallback;
    private TeletextPacketCallback packetCallback;
    private TeletextServiceInfoCallback serviceInfoCallback;
      ...
      for (int i = 0; dataLeft >= size; i++)
      {
        ...
        if (!(data_unit_id == 0xFF || data_unit_id == 0x02 || data_unit_id == 0x03))
        {
          ...
          assert(data_unit_id == 0xFF || data_unit_id == 0x02 || data_unit_id == 0x03,
                 "Data unit id invalid value " + data_unit_id);
          return;
        }
